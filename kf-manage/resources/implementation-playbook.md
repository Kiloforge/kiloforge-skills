...
Run `~/.kf/bin/kf-track.py list` and scan directories:
...
- Run `~/.kf/bin/kf-track.py list` to get all tracks
- For each track with status `pending` or `in-progress`:
  - Run `~/.kf/bin/kf-track.py get {trackId}` for type, dates
  - Run `~/.kf/bin/kf-track-content.py progress {trackId}` for task counts
...
Read `.agent/kf/tracks/{track-id}/track.yaml` (or run `~/.kf/bin/kf-track.py get {track-id}`):
...
3. Update `.agent/kf/tracks.yaml` via:
   ```bash
   ~/.kf/bin/kf-track.py update {track-id} --status completed
   ```
